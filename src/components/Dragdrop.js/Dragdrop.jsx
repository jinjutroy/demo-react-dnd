...
function DragDrop() {
  ...
  const saveToData =  () => {
    ...
    fetch("https://api-demo-nextjs.herokuapp.com/api/data/"+config.id, requestOptions)
      .then((response) => response.json())
      .then((data) => {
        data.result.shift();
        setBoard(data.result);
  ...
  const showDialog = (e) => {
    ...
    let temp = {
      id: id, 
      component: element.type === "button" ? "ElementButton" : "ElementParagraph",
      props: {},
    };
    if (element.text !== "") {
  ...
  const handlerUndo = ()=>{ 
    ...
    if(element?.action === "CREATE"){
      ...
      let temp = {
        ...
        type: element.data?.component === "ElementParagraph" ? "paragraph" : "button",
      }
      addToData(temp);
    }
  }
  ...
  return (
    <div className="container">
      <div className="nav-bar">
        <div onClick={() => saveChange()} className="nav-bar__item absnav-bar__save">Save</div>
        <div onClick={() => handlerUndo()} className={isUndo?"nav-bar__item nav-bar__undo ":" nav-bar__item nav-bar__undo unactive"}>Undo</div>
        <div onClick={() => handlerRedo()} className={isRedo?"nav-bar__item nav-bar__redo":" nav-bar__item nav-bar__redo unactive"}>Redo</div>
        <div onClick={() => handlerExport()}  className="nav-bar__item nav-bar__export">Export</div>
        <div onClick={() => handlerImport()}className="nav-bar__item nav-bar__import">Import</div>
        <Link
      ...
      <div className="wrapper">
        <div className="wrapper-board">
          ...
          <div className="Board" onMouseMove={(e) => getLocation(e)} ref={drop}>
            ...
            <Board board={board} text={text} showDialog={(value) => showDialog(value)} />
              ...
              <div className={btnClick ? "change-info show" : " hide"}>
                <div className="wrap-button">
                  <div className="">
                    <div>
                      <div>Button Text</div>
                      <input  onFocus={(e) => e.target.value = ""} onChange={e =>updateText(e)} type="text" />
                    </div>
                    <div> 
                      <div>Alert Message</div>
                      <input onFocus={(e) => e.target.value = ""} onChange={e =>updateAlertMessage(e)}  type="text" />
                    </div>
                  </div>
                <button  onClick={e =>deleteElement(e)} className="button-delete" type="button">Delete</button>
                </div>
              </div>
